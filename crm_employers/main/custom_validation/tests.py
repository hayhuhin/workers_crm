...
class TestAPIData:
    ...
    def create_finance_mock_data(self,company_name):
...
class TestAPIData:
    ...
    def create_customer_mock_data(self,company_name,another_company_name):
            company_obj = Company.objects.get(name=company_name)
            second_company_obj = Company.objects.get(name=another_company_name)
            customer_names = ["radco1","radco2","radco3","radco4","radco5"]
            another_customer_names = ["django1","django2","django3","django4","django5"]
            ...
            #*creating another company customer
            for index,name in enumerate(customer_names):
                customer_obj = Customer.objects.create(
                name=name,
                email=f"{name}@{name}.com",
    ...
    def create_finance_mock_data(self,company_name,another_company_name):
            company_obj = Company.objects.get(name=company_name)
            second_company_obj = Company.objects.get(name=another_company_name)
            
            date_list = ["2024-01-01","2024-02-01","2024-03-01","2024-04-01","2024-05-01"]
            amount = ["1234","2341","3412","4123","1234"]
            customer_list = company_obj.customer_set.all()

            for counter,user in enumerate(company_obj.user_set.all()):
                income_obj = Income.objects.create(
                    user=user,
                    amount=amount[counter],
                    date_received=date_list[counter],
                    description=f"{user.username} description",
                    payment_method="credit_card",
                    customer=customer_list[counter]
                    )
                income_obj.save()
    
            date_list = ["2024-01-01","2024-02-01","2024-03-01","2024-04-01","2024-05-01"]
            amount = ["5678","6785","7856","8567","5678"]
            customer_list = second_company_obj.customer_set.all()

            for counter,user in enumerate(second_company_obj.customer_set.all()):
